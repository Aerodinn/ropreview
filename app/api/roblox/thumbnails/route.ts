...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [gameRes, thumbRes, iconRes] = await Promise.all([
      fetch(`https://games.roblox.com/v1/games?universeIds=${universeId}`, { headers: H }),
      fetch(`https://thumbnails.roblox.com/v1/games/multiget/thumbnails?` +
        new URLSearchParams({ universeIds: universeId, countPerUniverse: "1", defaults: "true", size: "768x432", format: "Png", isCircular: "false" }),
        { headers: H }),
      fetch(`https://thumbnails.roblox.com/v1/games/icons?` +
        new URLSearchParams({ universeIds: universeId, returnPolicy: "PlaceHolder", size: "150x150", format: "Png", isCircular: "false" }),
        { headers: H }),
    ]);
